...
class LiveTradingCoordinator:
    ...
    def __init__(
        ...
        self._timeframe_delta = timedelta(minutes=self._interval_to_minutes(config.timeframe))
        self._candle_delay = timedelta(seconds=config.candle_ready_delay_seconds)
        
        # Initialize components
        self._position_manager = PositionManager(config, exchange, logger, telegram_client=telegram_client)
    ...
    def _latest_ready_time(self, current_time: datetime) -> datetime:
        ...
        periods_since_midnight = int(elapsed.total_seconds() // self._timeframe_delta.total_seconds())
    ...
    def _execute_strategy_cycle(self, current_time: datetime) -> None:
        ...
        try:
            ...
            # 4. Execute signals
            self._log.info(f"Executing {len(signals)} trading signals")
            successful = 0
