...
class LiveTradingStrategy:
    ...
    def analyze_symbol(
        ...
        try:
            ...
            klines = klines[:-1]  # Drop last candle to avoid in-progress data
            ...
            last_closed_ha_candle = ha_candles[-1]
            self._log.debug(
            ...
            self._log.debug(
                ...
                symbol,
                "Green" if ha_candles[-2].is_bullish() else "Red",
                ha_candles[-2].ha_open,
                ha_candles[-2].ha_high,
                ha_candles[-2].ha_low,
                ha_candles[-2].ha_close,
            )
            self._log.debug(
                "%s HA candle[-3] (%s): open=%.6f, high=%.6f, low=%.6f, close=%.6f",
                symbol,
                "Green" if ha_candles[-3].is_bullish() else "Red",
                ha_candles[-3].ha_open,
                ha_candles[-3].ha_high,
                ha_candles[-3].ha_low,
                ha_candles[-3].ha_close,
            )
    ...
    def generate_signals(
        ...
        for symbol_info in top_losers:
            ...
            if signal is not None and signal.side == PositionSide.LONG:
        ...
        for symbol_info in top_gainers:
            signal = self.analyze_symbol(symbol_info, current_time)
            if signal is not None and signal.side == PositionSide.SHORT:
