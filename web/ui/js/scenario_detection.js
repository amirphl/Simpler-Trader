(() => {
  ...
  const penetrationTbody = document.getElementById("penetration-tbody");
  ...
  const penetrationCountEl = document.getElementById("penetration-count");
  ...
  const scenarioCountBadgeEl = document.getElementById("scenario-count-badge");
  const penetrationCountBadgeEl = document.getElementById(
    "penetration-count-badge",
  );
  ...
  const showScenariosInput = document.getElementById("show-scenarios");
  const showPenetrationsInput = document.getElementById("show-penetrations");
  ...
  const COLORS = {
    ...
    scenarioBull: "#8ddcff",
    scenarioBear: "#ff9f43",
    penetration: "rgba(255, 255, 255, 0.68)",
  };
  ...
  function scenarioStyle(scenario) {
    const bullish = scenario.direction === "BULLISH";
    return {
      color: bullish ? COLORS.scenarioBull : COLORS.scenarioBear,
      fill: bullish ? "rgba(248, 214, 109, 0.18)" : "rgba(255, 138, 122, 0.18)",
      label: bullish ? "BULL" : "BEAR",
    };
  }

  function penetrationColor(penetration) {
    return penetration.side === "from_below" ? COLORS.up : COLORS.down;
  }
  ...
  function drawScenarioMarker(x, y, color, direction) {
    overlayCtx.save();
    ...
    overlayCtx.stroke();
    overlayCtx.fillStyle = color;
    overlayCtx.font = "800 12px 'IBM Plex Mono', monospace";
    overlayCtx.textAlign = "center";
    overlayCtx.textBaseline = "middle";
    overlayCtx.fillText(direction === "BULLISH" ? "B" : "S", x, y + 0.5);
    overlayCtx.restore();
  }

  function drawScenarios() {
    if (showScenariosInput?.checked === false) return;
    lastScenarios.forEach((scenario) => {
      const zoneHunt = scenario.zone_hunt;
      const pivotHunt = scenario.confirming_pivot_hunt;
      const x0 = timeToX(zoneHunt.time);
      const x1 = timeToX(pivotHunt.time);
      const y0 = priceToY(zoneHunt.line_price);
      const y1 = priceToY(pivotHunt.price);
      ...
      drawScenarioMarker(x1, y1, style.color, scenario.direction);
      drawTag(
        `${style.label} ${pivotHunt.candle_index}`,
        x1 + 10,
        Math.max(y1 - 26, 24),
        style.color,
        "rgba(8, 18, 25, 0.94)",
  ...
  function renderScenarioTable(scenarios) {
    ...
    scenarios.forEach((scenario, idx) => {
      ...
      appendCell(row, scenario.id, { color: style.color });
      appendCell(row, scenario.direction === "BULLISH" ? "BULL" : "BEAR", {
        color: style.color,
      });
      appendCell(row, `${scenario.zone_id} L${scenario.zone_level}`, {
        className: "td-muted",
      });
      appendCell(row, scenario.zone_hunt.candle_index, {
        className: "td-muted",
      });
      appendCell(row, scenario.confirming_pivot_hunt.candle_index);
      appendCell(
        row,
        `${pivotTypeBadge(scenario.confirming_pivot_hunt.pivot_type)} ${scenario.confirming_pivot_hunt.pivot_index}`,
        { className: "td-muted" },
      );
      appendCell(row, scenario.confirming_pivot_hunt.price.toFixed(4), {
        className: "td-muted",
      });
      ...
      row.addEventListener("click", () => {
        ...
        const huntTime = unixTime(scenario.zone_hunt.time);
        const confirmTime = unixTime(scenario.confirming_pivot_hunt.time);
        const center = Math.floor((huntTime + confirmTime) / 2);
        const span = Math.max(Math.abs(confirmTime - huntTime), 8 * 3600);
        focusTimeWindow(center, Math.ceil(span * 0.7));
      });
  ...
  function renderPenetrationTable(penetrations) {
    if (!penetrationTbody) return;
    penetrationTbody.innerHTML = "";
    penetrations.forEach((penetration, idx) => {
      const row = document.createElement("tr");
      row.dataset.idx = String(idx);
      const color = penetrationColor(penetration);
      appendCell(row, penetration.candle_index);
      appendCell(
        row,
        penetration.side === "from_below" ? "from below" : "from above",
        {
          color,
        },
      );
      appendCell(row, `${penetration.zone_id} L${penetration.zone_level}`, {
        className: "td-muted",
      });
      appendCell(row, penetration.zone_direction === "UPWARD" ? "UP" : "DN", {
        className:
          penetration.zone_direction === "UPWARD" ? "td-up" : "td-down",
      });
      appendCell(row, penetration.line_price.toFixed(4));
      appendCell(
        row,
        `${penetration.candle_open.toFixed(2)}/${penetration.candle_high.toFixed(2)}/${penetration.candle_low.toFixed(2)}/${penetration.candle_close.toFixed(2)}`,
        { className: "td-muted" },
      );
      ...
      row.addEventListener("click", () => {
        penetrationTbody
          ...
          .forEach((r) => r.classList.remove("selected"));
        row.classList.add("selected");
        focusTimeWindow(unixTime(penetration.time), 8 * 3600);
      });

      penetrationTbody.appendChild(row);
    });
  }
  ...
  function render(
    ...
    lastScenarios = scenarios.map((scenario) => ({
      ...scenario,
      zone_hunt: {
        ...scenario.zone_hunt,
        time: unixTime(scenario.zone_hunt.time),
      },
      confirming_pivot_hunt: {
        ...scenario.confirming_pivot_hunt,
        time: unixTime(scenario.confirming_pivot_hunt.time),
      },
      cancelling_pivot_hunt: scenario.cancelling_pivot_hunt
        ? {
            ...scenario.cancelling_pivot_hunt,
            time: unixTime(scenario.cancelling_pivot_hunt.time),
          }
        : null,
    }));
    lastPenetrations = penetrations.map((penetration) => ({
      ...penetration,
      time: unixTime(penetration.time),
    }));
    ...
    renderScenarioTable(lastScenarios);
    renderPenetrationTable(lastPenetrations);
    ...
    const penetrationCount = String(penetrations.length);
    const segCount = String(segments.length);
    const zCount = String(zones.length);
    if (scenarioCountEl) scenarioCountEl.textContent = scenarioCount;
    if (penetrationCountEl) penetrationCountEl.textContent = penetrationCount;
    ...
    if (scenarioCountBadgeEl) scenarioCountBadgeEl.textContent = scenarioCount;
    if (penetrationCountBadgeEl) {
      penetrationCountBadgeEl.textContent = penetrationCount;
    }
  ...
  async function handleSubmit(event) {
    ...
    statusEl.textContent =
      "Requesting candles, zones, penetrations, and scenarios...";
    ...
    const payload = {
      ...
      // ── Scenario detection ───────────────────────────────────────
      scenario_epsilon: Number(formData.get("scenario_epsilon") || 0),
      zone_selection: formData.get("zone_selection") || "all",
      bullish_pivot_selection:
        formData.get("bullish_pivot_selection") || "non_hunted_only",
      bearish_pivot_selection:
        formData.get("bearish_pivot_selection") || "non_hunted_only",
      mark_zone_hunted: formData.has("mark_zone_hunted"),
      deduplicate_by_zone_and_current: formData.has(
        "deduplicate_by_zone_and_current",
      ),
      current_candle_only: formData.has("current_candle_only"),
      current_index: currentIndexRaw ? Number(currentIndexRaw) : null,
    };
    ...
    try {
      ...
      statusEl.textContent =
        ...
        `${json.zones.length} liquidity lines, ` +
        `${(json.penetrations || []).length} penetrations, ` +
        `and ${(json.scenarios || []).length} scenarios.`;
    } catch (err) {
      console.error(err);
  ...
  showScenariosInput?.addEventListener("change", drawOverlay);
  showPenetrationsInput?.addEventListener("change", drawOverlay);
